...
import lemis.models.losses as losses
import lemis.models.optimizer as optim
import lemis.utils.checkpoint as cu
import lemis.utils.distributed as du
import lemis.utils.logging as logging
import lemis.utils.misc as misc

from lemis.datasets import loader
from lemis.models import build_model
from lemis.utils.meters import EpochTimer, SurgeryMeter
...
def train_epoch(
    ...
    loss_dict = {
        task: losses.get_loss_func(loss_funs[t_id])(reduction=cfg.SOLVER.REDUCTION)
    ...
    if cfg.REGIONS.ENABLE and cfg.TASKS.PRESENCE_RECOGNITION:
        pres_loss_dict = {
            f"{task}_presence": losses.get_loss_func("bce")(
                reduction=cfg.SOLVER.REDUCTION
            )
            for task in cfg.TASKS.PRESENCE_TASKS
        }
        pres_type_dict = {
            f"{task}_presence": losses.get_loss_type("bce")
            for task in cfg.TASKS.PRESENCE_TASKS
        }
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg):
    ...
    for cur_iter, (inputs, labels, data, image_names) in enumerate(val_loader):
        ...
        assert (not (cfg.REGIONS.ENABLE and cfg.FEATURES.ENABLE)) or len(
            rpn_ftrs
        ) == len(image_names) == len(
            boxes
        ...
        if cfg.NUM_GPUS:
            ...
            if cfg.NUM_GPUS > 1:
                ...
                image_names = torch.cat(
                    du.all_gather_unaligned(image_names), dim=0
                ).tolist()
                ...
                if cfg.REGIONS.ENABLE:
                    ...
                    for i in range(len(idxs_gather)):
                        idxs_gather[i] += torch.tensor(
                            (cfg.TEST.BATCH_SIZE / cfg.NUM_GPUS) * i
                        ).long()
        ...
        if cfg.REGIONS.ENABLE:
            for task in region_tasks:
                preds[task] = [
                    preds[task][boxes_idxs == idx].tolist()
                    for idx in range(len(boxes_mask))
                ]
            if "masks" in preds:
                ori_boxes = [
                    preds["boxes"][boxes_mask][boxes_idxs == idx].numpy().tolist()
                    for idx in range(len(boxes_mask))
                ]
                preds["masks"] = [
                    preds["masks"][boxes_mask][boxes_idxs == idx].numpy()
                    for idx in range(len(boxes_mask))
                ]
            else:
                ori_boxes = [
                    ori_boxes[boxes_idxs == idx].tolist()
                    for idx in range(len(boxes_mask))
                ]
...
def train(cfg):
    ...
    if cfg.TEST.ENABLE:
        ...
        map_task, mean_map, out_files = eval_epoch(
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        if not cfg.MODEL.KEEP_ALL_CHECKPOINTS:
            del_fil = os.path.join(
                cfg.OUTPUT_DIR,
                "checkpoints",
                "checkpoint_epoch_{0:05d}.pyth".format(cur_epoch - 1),
            )
            ...
            if (cfg.NUM_GPUS > 1 and du.is_master_proc()) or cfg.NUM_GPUS == 1:
                ...
                if mean_map > best_mean_map:
                    ...
                    for task in complete_tasks:
                        ...
                        copy_path = os.path.join(
                            best_preds_path, file.replace("epoch", "best_all")
                        )
                ...
                for task in complete_tasks:
                    if list(map_task[task].values())[0] > best_task_map[task]:
                        ...
                        file = out_files[task].split("/")[-1]
                        copy_path = os.path.join(
                            best_preds_path, file.replace("epoch", "best")
                        )
                        shutil.copyfile(out_files[task], copy_path)
                        cu.save_best_checkpoint(
                            cfg.OUTPUT_DIR,
