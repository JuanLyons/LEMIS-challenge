...
def detection_collate(batch):
    ...
    image_names = (
        image_names if type(image_names[0]) == str else torch.tensor(image_names)
    )
    ...
    for key in extra_data[0]:
        ...
        if key == "boxes_mask":
            ...
            idxs = list(
                itertools.chain(*[[d_id] * len(d) for d_id, d in enumerate(data)])
            )
    ...
    for key in all_labels[0]:
        ...
        assert all(
            type(data[0]) == type(d) for d in data
        ), f"Inconsistent data type {data}"

        if isinstance(data[0], list):
            data = list(itertools.chain(*data))
        else:
            # TODO: These are just security checks, REMOVE when all done
            assert isinstance(data[0], int) or isinstance(
                data[0], np.ndarray
            ), f"Type {type(data[0])} not supported"
