...
class SurgicalDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, split, load=True):
        ...
        if self._split == "train":
            ...
            self._use_color_augmentation = (
                cfg.ENDOVIS_DATASET.TRAIN_USE_COLOR_AUGMENTATION
            )
    ...
    def _images_and_boxes_preprocessing_cv2(self, imgs, boxes, image=None):
        ...
        if self._split == "train" and not self.cfg.DATA.JUST_CENTER:  # "train"
            ...
            boxes = [
                cv2_transform.scale_boxes(self._crop_size[0], boxes[0], height, width)
            ]
            ...
            crop_aspect_ratio = (
                self.cfg.DATA.TEST_CROP_SIZE_LARGE / self.cfg.DATA.TEST_CROP_SIZE
            )
            assert (
                image is None
