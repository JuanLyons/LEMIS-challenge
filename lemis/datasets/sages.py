...
class Sages(SurgicalDataset):
    """
    Sages dataloader.
    """

    def __init__(self, cfg, split, load=True):
        self.dataset_name = "sages"
        self.zero_fill = 5
        self.image_type = "jpg"
        self.fold = cfg["ENDOVIS_DATASET"]["FRAME_DIR"].split(os.sep)[-2].split("_")[-1]
        super().__init__(cfg, split, load)

    def keyframe_mapping(self, video_idx, sec_idx, sec):
        video_name = self._video_idx_to_name[video_idx]
        path_complete_name = self.frame_name_joining(video_name, sec)
        path_complete_name = f"./data/sages_{self.fold}/frames/" + path_complete_name
    ...
    def __getitem__(self, idx):
        ...
        assert (
            int(
                ...
                .split("/")[-1]
                .replace("." + self.image_type, "")
            )
            == sec
        ), f'Different {self._image_paths[video_idx][center_idx].split("/")[-1].replace("."+self.image_type,"")} {sec}'
