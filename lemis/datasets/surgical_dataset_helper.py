...
from lemis.utils.env import pathmgr
...
def load_features_boxes(cfg, split):
    ...
    for file, boxes in features.items():
        assert all(
            len(feats) == cfg.FEATURES.DIM_FEATURES for feats in boxes.values()
...
def load_image_lists(cfg, is_train):
    ...
    list_filenames = [
        os.path.join(
            cfg.ENDOVIS_DATASET.FRAME_LIST_DIR,
            (
                cfg.ENDOVIS_DATASET.TRAIN_LISTS
                if is_train
                else cfg.ENDOVIS_DATASET.TEST_LISTS
            ),
        )
    ]
        ...
        with pathmgr.open(list_filename, "r") as f:
            for line in f:
                ...
                image_paths[data_key].append(
                    os.path.join(cfg.ENDOVIS_DATASET.FRAME_DIR, row[3])
                )
            ...
            if verify_annots(annotation, cfg, filter):
                ...
                if cfg.REGIONS.ENABLE:
                    ...
                    try:
                        assert bbox not in [
                            item["bbox"]
                            for item in annotated_frames_dict[video_name][frame_num]
                        ], f"bbox {bbox} is reapeted in frame {frame_num} of video {video_name}"
                    except AssertionError:
                        logger.warning(
                            f"Repeated bounding box in dataset, check if this is ok and comment this line if so."
                        )
