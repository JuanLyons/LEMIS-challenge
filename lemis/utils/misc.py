...
import lemis.utils.multiprocessing as mpu
from lemis.models.batchnorm_helper import SubBatchNorm3d
from lemis.datasets.utils import pack_pathway_output
...
def _get_model_analysis_input(cfg, use_input_frames):
    ...
    if use_input_frames:
        ...
        final_dim = int(
            cfg.MVIT.EMBED_DIM
            * math.prod([dim_mul[-1] for dim_mul in cfg.MVIT.DIM_MUL])
        )
        time_resolution = cfg.DATA.NUM_FRAMES // cfg.MVIT.PATCH_STRIDE[0]
        h_resolution = (
            cfg.DATA.TEST_CROP_SIZE // cfg.MVIT.PATCH_STRIDE[1]
        ) // math.prod([q_pool[-1] for q_pool in cfg.MVIT.POOL_Q_STRIDE])
        w_resolution = (
            cfg.DATA.TEST_CROP_SIZE_LARGE // cfg.MVIT.PATCH_STRIDE[2]
        ) // math.prod([q_pool[-1] for q_pool in cfg.MVIT.POOL_Q_STRIDE])

        input_tensors = torch.rand(
            1, time_resolution * h_resolution * w_resolution, final_dim
        )
    ...
    if cfg.REGIONS.ENABLE:
        max_boxes = (
            cfg.DATA.MAX_BBOXES * 2
            if cfg.ENDOVIS_DATASET.INCLUDE_GT
            else cfg.DATA.MAX_BBOXES
        )
