...
import lemis.evaluate.main_eval as grasp_eval
import lemis.utils.logging as logging
import lemis.utils.misc as misc
from lemis.evaluate.utils import mask_to_rle
...
from lemis.datasets import DATASET_REGISTRY
...
class SurgeryMeter(object):
    ...
    def __init__(self, overall_iters, cfg, mode):
        ...
        # if self.parallel:
        self.join_funct = DATASET_REGISTRY.get(self.dataset_name.capitalize())(
            cfg, "test", False
        ).frame_num_joining
        ...
        if cfg.REGIONS.ENABLE:
            self._region_tasks = {
                ...
                if task in cfg.ENDOVIS_DATASET.REGION_TASKS
            }
        ...
        if self.segmentation and not self.generate_masks:
            with open(
                os.path.join(
                    cfg.ENDOVIS_DATASET.ANNOTATION_DIR,
                    cfg.ENDOVIS_DATASET.TEST_PREDICT_BOX_JSON,
                )
            ) as f:
    ...
    def update_stats(self, preds, names, boxes, final_loss=None, losses=None, lr=None):
        ...
        if self.eval_train or self.mode in ["val", "test"]:
            ...
            if self.parallel:
                try:
                    names = [
                        self.join_funct(name[0], name[1]) for name in names
                    ]  # [IDENT_FUNCT_DICT[self.dataset_name](*name) for name in names]
                except:
                    print(names)
                    raise
            ...
            if self.regions:
                ...
                assert all(
                    len(names) == len(boxes) == len(preds[t]) for t in self.tasks
                )
                if "masks" in preds:
                    for masks in preds["masks"]:
                        self.all_masks.append(
                            [mask_to_rle(mask.astype("uint8")) for mask in masks]
                        )

                    assert all(
                        len(names) == len(boxes) == len(preds["masks"]) == len(preds[t])
                        for t in self.tasks
                    )
            else:
    ...
    def replace_nan(self, dictionary):
        if np.isnan(list(dictionary.values())).any():
            result = {
                k: (v if not np.isnan(v) else str(v)) for k, v in dictionary.items()
            }
            return result
        else:
            return dictionary
    ...
    def finalize_metrics(self, epoch, log=True):
        ...
        for task, metric in zip(self.tasks, self.metrics):
            ...
            self.full_map[task] = grasp_eval.main_per_task(
                self.groundtruth,
                out_name[task],
                task,
                metric,
                masks_path=self.mask_path,
            )
            if log:
                stats = {"mode": self.mode, "task": task, "metric": self.full_map[task]}
                stats["metric"] = self.replace_nan(stats["metric"])
                logging.log_json_stats(stats)
        if log:
            stats = {
                "mode": self.mode,
                "mean metric": np.mean(
                    [v[m] for v, m in zip(list(self.full_map.values()), self.metrics)]
                ),
            }
            logging.log_json_stats(stats)
        ...
        return (
            self.full_map,
            np.mean([v[m] for v, m in zip(list(self.full_map.values()), self.metrics)]),
            out_name,
        )
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        if self.mode in ["val", "test"]:
            ...
            for idx, task in enumerate(self.tasks):
                self.full_map[task] = self.replace_nan(self.full_map[task])
    ...
    def save_json(self, task, epoch):
        ...
        if self.regions:
            assert (
                len(preds[task]) == len(names) == len(boxes)
            ), f"Inconsistent lengths {len(preds[task])} {len(names)} {len(boxes)}"
            if self.generate_masks:
                masks = self.all_masks
                assert (
                    len(preds[task]) == len(names) == len(boxes) == len(masks)
                ), f"Inconsistent lengths {len(preds[task])} {len(names)} {len(boxes)} {len(masks)}"
        else:
            assert len(preds[task]) == len(
                names
            ), f"Inconsistent lengths {len(preds[task])} {len(names)}"
                ...
                if self.segmentation:
                    save_json_dict[name] = {
                        "instances": [
                            {
                                "bbox": box,
                                task_key_name: pred[b_id],
                                "segment": (
                                    self.region_proposals[name][tuple(box)]
                                    if not self.generate_masks
                                    else masks[idx][b_id]
                                ),
                            }
                            for b_id, box in enumerate(boxes[idx])
                            if box != [0, 0, 0, 0]
                        ]
                    }
                else:
                    save_json_dict[name] = {
                        "instances": [
                            {"bbox": box, task_key_name: pred[b_id]}
                            for b_id, box in enumerate(boxes[idx])
                            if box != [0, 0, 0, 0]
                        ]
                    }
        ...
        path_prediction = os.path.join(
            self.output_dir, f"epoch_{epoch}_preds_{task}.json"
        )
