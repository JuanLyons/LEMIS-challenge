...
import lemis.utils.weight_init_helper as init_helper
...
class MViT(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self._frame_tasks = {
            ...
            if task not in cfg.ENDOVIS_DATASET.REGION_TASKS
        }
                ...
                if self.cls_embed_on:
                    self.pos_embed_class = nn.Parameter(
                        torch.zeros(
                            ...
                            (
                                1
                                if not self.multiple_cls_embeds
                                else len(self._frame_tasks)
                            ),
                            embed_dim,
                        )
                    )
    ...
    def forward(
        ...
        for task in self.tasks:
            ...
            if self.recogn and task in self.recog_tasks:
                out[f"{task}_presence"] = getattr(
                    self, "extra_heads_{}_presence".format(task)
...
class SwinV2(nn.Module):
    def __init__(self, cfg):
        ...
        self.proj_features = nn.Linear(9216, 768)

        self.proj_seq = nn.Linear(128, 98)
    ...
    def forward(
        ...
        x = [
            torch.load(
                "/home/dlmanrique/Endovis/CVS_Challenge/SOTAS/SwinCVS-Challenge-CVS-2025/features/patch_embeddings/"
                + image_name.replace("jpg", "pt")
            ).to("cuda")
        ...
        x = self.proj_seq(x.transpose(1, 2)).transpose(1, 2)
